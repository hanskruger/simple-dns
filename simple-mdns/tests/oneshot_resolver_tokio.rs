...
#[tokio::test]
async fn one_shot_resolver_address_query_bind_to_if_ipv4() {
    let _responder = get_oneshot_responder(Name::new_unchecked("_async._tcp.local")).await;
    tokio::time::sleep(Duration::from_millis(500)).await;

    let mut resolver = OneShotMdnsResolver::new_with_scope(NetworkScope::V4WithInterface(Ipv4Addr::new(127,0,0,1))).expect("Failed to create resolver");
    resolver.set_unicast_response(false);
    let answer = resolver.query_service_address("_async._tcp.local").await;
